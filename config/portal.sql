...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 05, 2025 at 09:18 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `student_count`  AS SELECT 1 AS `total_students` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `user_count`  AS SELECT 1 AS `user_type`, 1 AS `total_users` ;
