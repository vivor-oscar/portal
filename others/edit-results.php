...
<body class="bg-gray-100">
    <div class="container mx-auto p-4">
        ...
        <div class="flex justify-between mb-4">
            ...
            <input type="file" id="pdfImport" accept=".pdf" class="hidden" onchange="importPDF(event)">
            <button onclick="document.getElementById('pdfImport').click()" class="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">Import PDF</button>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (typeof window.pdfjsLib !== 'undefined') {
                ...
                // If pdfjsLib is not loaded yet, wait for it
        ...
        async function importPDF(event) {
            ...
            try {
                ...
                const studentData = {
                    studentName: extractValue(text, 'Name:', ','),
                    rollNumber: extractValue(text, 'No. on Roll:', ','),
                    studentClass: extractValue(text, 'Class:', ','),
                    term: extractValue(text, 'Term:', ','),
                    academicYear: extractValue(text, 'Academic Year:', ','),
                    totalScore: extractValue(text, 'Total Score:', ','),
                    position: extractValue(text, 'Position:', ','),
                    termClosingDate: extractValue(text, 'Term Closing On:', ','),
                    aggregate: extractValue(text, 'Aggregate:', ','),
                    nextTermBegins: extractValue(text, 'Next Term Begins:', ','),
                    attendance: extractValue(text, 'Attendance:', ','),
                    promotedTo: extractValue(text, 'Promoted To:', ','),
                    conduct: extractValue(text, 'Conduct:', ','),
                    attitude: extractValue(text, 'Attitude:', ','),
                    interest: extractValue(text, 'Interest:', ','),
                    teacherRemarks: extractValue(text, "Class Teacher's Remarks:", '\n')
                };
                ...
                populateForm(studentData, subjects);

                alert('PDF imported successfully!');
            } catch (error) {
                console.error('Error importing PDF:', error);
                alert('Error importing PDF. Please make sure it\'s a valid report sheet PDF.');
            }
        }
        ...
        function extractValue(text, startKey, endKey) {
            ...
            const valueEnd = text.indexOf(endKey, valueStart);
            
            return text.substring(valueStart, valueEnd === -1 ? text.length : valueEnd).trim();
        }
        ...
        function extractSubjects(text) {
            ...
            const subjectLines = text.split('\n').filter(line => 
                ...
                line.includes('STUDIES') ||
                line.includes('COMPUTING') ||
                line.includes('TWI') ||
                line.includes('RELIGIOUS') ||
        ...
        function populateForm(data, subjects) {
            ...
            // Add subject rows with extracted data
