...
<div class="flex min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        ...
        <div class="overflow-x-auto shadow rounded-lg border border-gray-300 dark:border-gray-700">
            <table class="min-w-full bg-white dark:bg-gray-800">
                <thead class="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200">
                    <tr>
                        <th class="px-4 py-2 border"><input type="checkbox" id="select-all" class="w-4 h-4" /></th>
                ...
                <tbody>
                    <tr>
                        <td colspan="6" class="px-4 py-3 bg-gray-50 dark:bg-gray-800">
                            <button id="promote-selected" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded">Promote selected</button>
                            <span id="promote-status" class="ml-3 text-sm text-gray-600 dark:text-gray-300"></span>
                        </td>
                    </tr>
                    <?php if ($students_result && mysqli_num_rows($students_result) > 0): ?>
                        <?php while ($student = $students_result->fetch_assoc()): ?>
                            <tr class="border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-4 py-2 border"><input type="checkbox" class="promote-checkbox w-4 h-4" value="<?= htmlspecialchars($student['student_id']) ?>" /></td>
                                ...
                                <td class='py-3 px-4'>
                                    <?php
                                        // Show promotion status if present
                                        $ps = '';
                                        $pRes = $conn->query("SELECT promotion_status FROM students WHERE student_id='" . $conn->real_escape_string($student['student_id']) . "' LIMIT 1");
                                        if ($pRes && $prow = $pRes->fetch_assoc()) $ps = $prow['promotion_status'];
                                        if ($ps === 'pending') {
                                            echo "<span class='px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full'>Pending</span>";
                                        } elseif ($ps === 'success') {
                                            echo "<span class='px-2 py-1 text-xs font-medium bg-green-100 text-green-800 rounded-full'>Success</span>";
                                        } elseif ($ps === 'failed') {
                                            echo "<span class='px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full'>Failed</span>";
                                        } else {
                                            echo "<span class='px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full'>None</span>";
                                        }
                                    ?>
                                </td>
                            </tr>
    ...
    (function(){
        ...
        var promoteBtn = document.getElementById('promote-selected');
        var statusSpan = document.getElementById('promote-status');

        if (selectAll) {
            selectAll.addEventListener('change', function(){
                var boxes = document.querySelectorAll('.promote-checkbox');
        ...
        if (promoteBtn) {
            promoteBtn.addEventListener('click', function(){
                var boxes = Array.from(document.querySelectorAll('.promote-checkbox:checked'));
                if (boxes.length === 0) { alert('Please select at least one student to promote.'); return; }
                if (!confirm('Are you sure you want to promote the selected students?')) return;
                promoteBtn.disabled = true;
                statusSpan.textContent = 'Promoting...';
                var fd = new FormData();
                boxes.forEach(function(b){ fd.append('student_ids[]', b.value); });
                fetch('../../controller/promote_students.php', { method: 'POST', body: fd })
                    .then(function(res){ return res.json(); })
                    .then(function(data){
                        if (data && data.status === 'ok') {
                            statusSpan.textContent = 'Promoted: ' + (data.updated || 0) + ((data.errors && data.errors.length) ? ' (with errors)' : '');
                            setTimeout(function(){ location.reload(); }, 1100);
                        } else {
                            statusSpan.textContent = 'Error: ' + (data.message || 'Unknown');
