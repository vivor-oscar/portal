...
// Fetch current student data using student_id from session if available, otherwise fall back to username
$student = null;
if (!empty($_SESSION['student_id'])) {
    $esc_sid = $conn->real_escape_string($_SESSION['student_id']);
    $result = mysqli_query($conn, "SELECT * FROM students WHERE student_id = '$esc_sid' LIMIT 1");
    $student = $result ? mysqli_fetch_assoc($result) : null;
}
if (!$student) {
    $esc_username = $conn->real_escape_string($current_username);
    $result = mysqli_query($conn, "SELECT * FROM students WHERE username = '$esc_username' LIMIT 1");
    $student = $result ? mysqli_fetch_assoc($result) : null;
}
...
if (!$student) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Determine the canonical student_id to use (prefer session value)
        $sid_raw = isset($_SESSION['student_id']) ? $_SESSION['student_id'] : $student['student_id'];
        ...
        if (empty($error) && $username !== $current_username) {
            ...
            $res = mysqli_query($conn, "SELECT student_id FROM students WHERE username = '$esc_new_username' AND student_id != '$esc_sid' LIMIT 1");
        ...
        if (empty($error) && $email !== $student['email']) {
            ...
            $res = mysqli_query($conn, "SELECT student_id FROM students WHERE email = '$esc_new_email' AND student_id != '$esc_sid' LIMIT 1");
            if ($res && mysqli_fetch_assoc($res)) {
        ...
        if (empty($error)) {
            ...
            // Build UPDATE using core mysqli with properly escaped values to update only the intended row
            ...
            $update_sql = "UPDATE students SET first_name='$esc_first', mid_name='$esc_mid', last_name='$esc_last', number='$esc_number', email='$esc_email', username='$esc_username'";
            if (!empty($new_password)) {
                $hashed = password_hash($new_password, PASSWORD_BCRYPT);
                $esc_hashed = $conn->real_escape_string($hashed);
                $update_sql .= ", password='$esc_hashed'";
            }
            // Use the session-backed student id (string) to ensure we update only the intended user
            $update_sql .= " WHERE student_id='" . $esc_sid . "'";
            ...
            if (mysqli_query($conn, $update_sql)) {
                ...
                // Refresh student data by student_id
                $res2 = mysqli_query($conn, "SELECT * FROM students WHERE student_id='$esc_sid' LIMIT 1");
                $student = $res2 ? mysqli_fetch_assoc($res2) : $student;
            } else {
